...
class FItSNEModel(Model):
    ...
    @classmethod
    def strategy_flex_mean(cls, data: Iterable, params: FItSNEParams):
        """"This strategy uses data from the previous layout
            as initialisation for the next.
            New points are mean of previous kNN
        """
        yield from cls._strategy_flex(cls.get_init_mean, data, params)

    @classmethod
    def strategy_flex_mean_weighted(cls, data: Iterable, params: FItSNEParams):
        """"This strategy uses data from the previous layout
            as initialisation for the next.
            New points are weighted (by distance) mean of previous kNN
        """
        yield from cls._strategy_flex(cls.get_init_mean_weighted, data, params)

    @classmethod
    def strategy_flex_median(cls, data: Iterable, params: FItSNEParams):
        """"This strategy uses data from the previous layout
            as initialisation for the next.
            New points are median of previous kNN
        """
        yield from cls._strategy_flex(cls.get_init_median, data, params)
