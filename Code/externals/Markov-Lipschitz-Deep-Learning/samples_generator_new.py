...
from sklearn.utils import check_array, check_random_state, shuffle as util_shuffle
...
def make_swiss_roll(n_samples=100, noise=0.0, random_state=None, remove=False, scale=1.0, center=None, r=None):
    ...
    if remove == 'star':
        ...
        def isInstar(center, r, XX):
            ...
            list_zuobiao_out = [(r * math.cos((90 + 72 * x) / 180 * 3.14), r * math.sin((90 + 72 * x) / 180 * 3.14)) for
                                x in [0, 1, 2, 3, 4]]
            r = (r * math.sin((18) / 180 * 3.14)) / math.sin((126) / 180 * 3.14)
            list_zuobiao_in = [(r * math.cos((126 + 72 * x) / 180 * 3.14), r * math.sin((126 + 72 * x) / 180 * 3.14))
                               for x in [0, 1, 2, 3, 4]]
    ...
    if remove == 'fivecircle':
        ...
        while num < n_samples:
            ...
            Flag = 0
            for i in range(5):
                if ((y - center[i, 1]) ** 2 + (v - center[i, 0]) ** 2) ** 0.5 < 21 * 0.5 and (
                        (y - center[i, 1]) ** 2 + (v - center[i, 0]) ** 2) ** 0.5 > 21 * 0.15:
                    Flag = 1
