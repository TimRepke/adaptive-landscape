...
from externals.Markov_Lipschitz_Deep_Learning.samples_generator_new import make_swiss_roll
...
def LoadData(data_name='SwissRoll', data_num=1500, seed=0, noise=0.0,
             device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'), remove=None, test=False):
    """
    ...
    if data_name == 'SwissRoll':
        if remove is None:
            ...
            train_data, train_label = make_swiss_roll(n_samples=data_num, noise=noise, random_state=seed + 1,
                                                      remove=remove, center=[10, 10], r=8)
