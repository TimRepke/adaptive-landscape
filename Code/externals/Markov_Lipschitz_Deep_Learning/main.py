...
import externals.Markov_Lipschitz_Deep_Learning.dataset as dataset
from externals.Markov_Lipschitz_Deep_Learning.model import MLDL_model
from externals.Markov_Lipschitz_Deep_Learning.loss import MLDL_Loss
from externals.Markov_Lipschitz_Deep_Learning.utils import *
...
def Generation(model, latent_point, label_point, latent_index):
    ...
    gif_ploter.Plot_Generation(latent_point[0], latent_point[latent_index], latent_point[-1], gen_latent, gen_data,
                               label_point, title=path + '/Generation.png')
...
def Generalization(Model, path):
    ...
    InlinePlot(Model, param['BATCHSIZE'], test_data, test_label, path=path, name='Test', indicator=False,
               mode=param['Mode'])
...
def SetParam():
    ...
    parser.add_argument("-D", "--DATASET", default='SwissRoll', type=str,
                        choices=['SwissRoll', 'SCurve', 'MNIST', 'Spheres5500', 'Spheres10000'])
    ...
    parser.add_argument("-GC", "--GradualChanging", default=[500, 1000], type=int,
                        nargs='+')  # Range for the gradual changing of push-away Loss
    parser.add_argument("-R", "--ratio", default=[0.2, 1.0, 0.0, 1.0], type=float,
                        nargs='+')  # The weight ratio for loss_ae/loss_iso/loss_angle/loss_push-away
    parser.add_argument("-EPS", "--Epsilon", default=0.23,
                        type=float)  # The boundary parameters used to determine the neighborhood
    ...
    if param['Train_MultiRun']:
        ...
        if param['DATASET'] == 'Spheres5500':
            optimizer_enc = torch.optim.Adam([{'params': [param for name, param in Model.named_parameters() if
                                                          any([s in name for s in param_enc])]}],
                                             lr=param['LEARNINGRATE'], weight_decay=1e-8)
        else:
            optimizer_enc = torch.optim.Adam([{'params': [param for name, param in Model.named_parameters() if
                                                          any([s in name for s in param_enc])]}],
                                             lr=param['LEARNINGRATE'])
        ...
        InlinePlot(Model, param['BATCHSIZE'], train_data, train_label, path, name='Train', indicator=True,
                   mode=param['Mode'])
        if param['DATASET'] != 'MNIST' or param['Visualization'] == True:
