...
def read_runtimes(path, n_runs=20):
    ...
    for pathname, filename in zip(run_paths, run_paths_json):
        if SacredRun.is_valid(filename):
            ...
            runtimes.append({'run': run.run_file, 'runtime_in_seconds': run.runtime, 'n_epochs': n_epochs,
                             'runtime_per_epoch_in_seconds': run_time_per_epoch})
...
def main():
    ...
    for i in np.arange(1, len(used)):
        sns.regplot(x='batch_size', y=used[i], data=df, label=labels[i - 1], color=colors[i - 1], truncate=False, ax=ax,
                    fit_reg=fit_reg)
